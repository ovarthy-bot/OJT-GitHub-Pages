...
- PDF önizleme sistemi düzeltildi:
  - Drive küçük resim bağlantılarına bağlı kalmaz.
  - PDF’in ilk sayfası uygulamaya dahil edilen PDF.js ile tarayıcıda oluşturulur.
  - Aynı anda en fazla iki önizleme işlenerek bellek ve işlemci kullanımı sınırlandırılır.
...
```bash
git add .
git commit -m "OJT uygulamasi v10"
git push
```
...
- PDF Dashboard küçük resimleri, Drive’dan indirilen PDF’in ilk sayfasının tarayıcıda yerel olarak işlenmesiyle oluşturulur; PDF içeriği başka bir servise gönderilmez.
