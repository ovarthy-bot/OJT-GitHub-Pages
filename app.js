...
const state = { records: [], connected: false, notesTimer: null, accessToken: null, tokenClient: null, driveContext: null, user: null, calendarMonth: '2026-07', activeView: localStorage.getItem('ojt_active_view')==='dashboard'?'dashboard':'records', dashboardTag: '', thumbnailUrls: new Map(), thumbnailPromises: new Map(), pdfJsPromise: null, thumbnailQueue: [], activeThumbnailJobs: 0 };
...
function loadPdfJs() {
  if(!state.pdfJsPromise){
    state.pdfJsPromise=import('./vendor/pdf.min.js').then(pdfjs=>{
...
function enqueueThumbnailJob(job) {
  return new Promise((resolve,reject)=>{
    state.thumbnailQueue.push({job,resolve,reject});
    runThumbnailQueue();
  });
}
function runThumbnailQueue() {
  while(state.activeThumbnailJobs<MAX_THUMBNAIL_JOBS&&state.thumbnailQueue.length){
    const item=state.thumbnailQueue.shift();
    state.activeThumbnailJobs+=1;
...
async function renderPdfThumbnail(blob) {
  ...
  try{
    ...
    const maxWidth=900,maxHeight=640;
    const scale=Math.min(maxWidth/baseViewport.width,maxHeight/baseViewport.height,2.2);
    const viewport=page.getViewport({scale:Math.max(scale,.35)});
    const pixelRatio=Math.min(window.devicePixelRatio||1,1.5);
    const canvas=document.createElement('canvas');
    canvas.width=Math.max(1,Math.floor(viewport.width*pixelRatio));
    canvas.height=Math.max(1,Math.floor(viewport.height*pixelRatio));
    const context=canvas.getContext('2d',{alpha:false});
    if(!context)throw new Error('Canvas başlatılamadı.');
    context.save();
    ...
    const thumbnailBlob=await new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',.86));
    page.cleanup();
    if(!thumbnailBlob)throw new Error('PDF önizlemesi oluşturulamadı.');
...
async function createDocumentThumbnail(record) {
  const documentId=record.document.id;
  let metadata=record.document||{};
  if(!metadata.mimeType||!metadata.name){
    const fetched=await driveFetch(`/files/${encodeURIComponent(documentId)}?fields=id,name,mimeType,webViewLink`);
  ...
  const response=await driveFetch(`/files/${encodeURIComponent(documentId)}?alt=media`);
  if(!(response instanceof Response))throw new Error('Belge verisi alınamadı.');
  const sourceBlob=await response.blob();
  if(!sourceBlob.size)throw new Error('Belge boş.');
  const mimeType=String(metadata.mimeType||sourceBlob.type||'').toLocaleLowerCase('tr-TR');
  const fileName=String(metadata.name||record.document?.name||'').toLocaleLowerCase('tr-TR');
  if(mimeType==='application/pdf'||fileName.endsWith('.pdf'))return renderPdfThumbnail(sourceBlob);
  if(mimeType.startsWith('image/')||sourceBlob.type.startsWith('image/'))return sourceBlob;
  throw new Error('Bu belge türü için önizleme desteklenmiyor.');
}
async function getThumbnailUrl(record) {
  ...
  const promise=enqueueThumbnailJob(async()=>{
    try{
      const blob=await createDocumentThumbnail(record);
      const url=URL.createObjectURL(blob);
      state.thumbnailUrls.set(documentId,url);
      return url;
    }catch(error){
      console.warn('PDF önizlemesi oluşturulamadı:',documentId,error);
      return '';
    }finally{
      state.thumbnailPromises.delete(documentId);
    }
...
function hydrateDashboardThumbnails(records) {
  ...
  records.forEach(async record=>{
    ...
    targets().forEach(target=>target.querySelector('.thumbnail-status')?.replaceChildren('PDF indiriliyor…'));
    const url=await getThumbnailUrl(record);
    targets().forEach(target=>{
      if(!target.isConnected)return;
      if(!url){target.classList.add('thumbnail-unavailable');target.querySelector('.thumbnail-status')?.replaceChildren('Önizleme oluşturulamadı');return}
      const image=document.createElement('img');
      image.src=url;image.alt=`${record.workOrder||record.document?.name||'PDF'} ilk sayfa önizlemesi`;image.loading='lazy';image.decoding='async';
